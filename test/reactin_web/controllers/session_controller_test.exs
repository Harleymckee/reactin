defmodule ReactinWeb.SessionController.Test do
  use ReactinWeb.ConnCase
  ...
  test "POST /session/new with invalid credentials", %{conn: conn} do
    conn = post conn, session_path(conn, :new), email: "blah", password: "blah"
  ...
  describe "POST /session/new with valid credentials" do
    test "logs the normal user in and redirects to the main menu", %{conn: conn} do
      ...
      conn = post conn, session_path(conn, :new), email: user.email, password: "blah"
